...
const Navbar = () => {
  ...
  return (
    <header
      className={`w-full sticky top-0 z-50 transition-all duration-300 ${scrolled ? "shadow-2xl" : "shadow-md"}`}
    >
      {/* --- Top Header --- */}
      ...
      <div className="hidden lg:block bg-primarys">
        <div className="mx-auto max-w-7xl flex items-center justify-between h-14 px-8">
                    ...
                    <div key={idx} className="group/item px-4 relative">
                      <div className="flex items-center justify-between py-3 px-4 rounded-xl hover:bg-primarys hover:text-white transition-all cursor-pointer">
                        <div className="flex items-center gap-4 font-bold">
                          <span className="text-texts-secondary group-hover/item:text-white transition-colors">
                            {cat.icon}
                          </span>
                          {cat.name}
                        </div>
                      ...
                      <div className="invisible opacity-0 group-hover/item:visible group-hover/item:opacity-100 absolute left-[95%] top-0 w-64 bg-white shadow-2xl rounded-3xl py-6 border border-gray-100 transition-all duration-300 scale-95 group-hover/item:scale-100">
                        ...
                        {cat.items.map((sub, i) => (
                          <Link
                            key={i}
                            href="#"
                            className="block px-8 py-2.5 text-sm text-texts-dark hover:text-primarys hover:pl-10 transition-all font-medium border-l-4 border-transparent hover:border-primarys"
                          >
                            {sub}
                          </Link>
                        ))}
          ...
          <Link
            href="/deals"
            className="flex items-center gap-2 bg-white text-primarys px-5 py-2 rounded-full text-xs font-black hover:bg-primarys-dark hover:text-white transition-all shadow-inner"
          >
      ...
      {/* --- Mobile Drawer Menu --- */}
      <div
        className={`fixed inset-0 z-60 lg:hidden transition-all duration-500 ${isMobileMenuOpen ? "visible" : "invisible"}`}
      >
        {/* Overlay */}
        <div
...
const NavItem = ({
  ...
  <div className="relative group px-5 h-full flex items-center cursor-pointer">
    <Link
      href={href}
      className="text-white font-black text-[12px] tracking-widest flex items-center gap-1 group-hover:opacity-80 transition-opacity"
    >
      {label}
      {hasSub && (
        <ChevronDown
    ...
    {hasSub && (
      <div className="invisible opacity-0 group-hover:visible group-hover:opacity-100 absolute top-full left-0 w-52 bg-white shadow-2xl rounded-b-2xl py-4 border-t-2 border-primarys transition-all duration-300 translate-y-2 group-hover:translate-y-0">
        <Link
          href="#"
          className="block px-6 py-2.5 text-sm text-texts-dark hover:bg-orange-50 hover:text-primarys font-bold"
        >
          New Tools
        </Link>
        <Link
