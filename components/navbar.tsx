      ...
      <div className="hidden lg:block bg-primarys text-white">
        <div className="mx-auto max-w-7xl flex items-center justify-between h-14 px-8">
                  ...
                  {categories.map((cat, idx) => (
                    <div key={idx} className="group/item px-4 relative">
                      ...
                      <div className="invisible opacity-0 group-hover/item:visible group-hover/item:opacity-100 absolute left-[95%] top-0 w-64 bg-white shadow-2xl rounded-3xl py-6 border border-gray-100 transition-all duration-300 scale-95 group-hover/item:scale-100">
                        <h4 className="px-8 text-[10px] font-black text-primarys uppercase tracking-widest mb-4">Popular Choices</h4>
                        {cat.items.map((sub, i) => (
                          <Link key={i} href={`/shop/${sub.toLowerCase().replace(/ /g, '-')}`} className="block px-8 py-2 text-sm text-texts-dark hover:text-primarys hover:pl-10 transition-all font-medium border-l-4 border-transparent hover:border-primarys">
                            {sub}
                          </Link>
                        ))}
                      </div>
          ...
          <div className="flex items-center gap-4">
            <Link href="/hot-deals" className="flex items-center gap-2 bg-white/20 hover:bg-white/30 text-white px-5 py-2 rounded-xl text-xs font-black tracking-widest transition-all">
               <Flame size={16} className="text-yellow-300 animate-pulse" /> HOT DEALS
            </Link>
            
            <Link href="/checkout" className="flex items-center gap-2 bg-white hover:bg-orange-200 text-primarys px-6 py-2 rounded-xl text-xs font-black tracking-widest transition-all shadow-lg hover:-translate-y-0.5 active:translate-y-0">
              ...
              {categories.map((cat, i) => (
                <details key={i} className="group mb-1">
                  ...
                  <div className="pl-14 pr-4 py-2 flex flex-col gap-3 border-l-2 border-orange-100 ml-8">
                    {cat.items.map((sub, j) => (
                      <Link key={j} href={`/shop/${sub.toLowerCase()}`} onClick={() => setIsMobileMenuOpen(false)} className="text-sm text-texts-secondary hover:text-primarys font-medium">
                        {sub}
                      </Link>
                    ))}
                  </div>
  ...
  <div className="relative group px-5 h-full flex items-center cursor-pointer">
    ...
    {hasSub && (
      <div className="invisible opacity-0 group-hover:visible group-hover:opacity-100 absolute top-full left-0 w-52 bg-white shadow-2xl rounded-b-2xl py-4 border-t-4 border-primarys transition-all duration-300 translate-y-2 group-hover:translate-y-0 text-texts-dark">
        ...
        <Link href="/shop/trending" className="block px-6 py-2.5 text-sm hover:bg-orange-50 hover:text-primarys font-bold">Best Sellers</Link>
        <Link href="/shop/clearance" className="block px-6 py-2.5 text-sm hover:bg-orange-50 hover:text-primarys font-bold">Clearance</Link>
      </div>
    )}
  </div>
);
